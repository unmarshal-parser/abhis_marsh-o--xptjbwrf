...
func (_Main *MainFilterer) ParseABIApprovalEvent(log types.Log, parsedLog ethrpc.Log) (*ABIApprovalEvent, error) {
	e := new(ABIApprovalEvent)
	if err := _Main.contract.UnpackLog(e, "Approval", log); err != nil {
		return nil, err
	}
	e.Raw = parsedLog
	return e, nil
